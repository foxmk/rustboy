...
impl Op {
    ...
    pub(in super) fn exec(&self, cpu: &mut Cpu) -> Result<bool, Error> {
        match self {
            Op::Load(dest, source) => {
            ...
            Op::Add(reg) => {
                cpu.set_byte_register(&Reg8::A, cpu.get_byte_register(&Reg8::A) + cpu.get_byte_register(reg));
                Ok(true)
            }
...
pub(in super) fn compile<Ops: IntoIterator<Item=Op>>(ops: Ops) -> Vec<u8> {
    let mut bytes = vec![];
